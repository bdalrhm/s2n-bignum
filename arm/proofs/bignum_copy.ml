...
let events = `\k z n x. APPEND
  (ENUMERATEL (k - MIN n k) (\i. [EventBranch ((MIN n k + i) + 1 < k); EventStore (word_add z (word (8 * ((MIN n k) + i))))]))
  (APPEND
    [EventBranch (k <= MIN n k)]
    (APPEND
      (ENUMERATEL (MIN n k) (\i. [EventBranch (i + 1 < MIN n k);
                                  EventStore (word_add z (word (8 * i)));
                                  EventLoad (word_add x (word (8 * i)))]))
      [EventBranch (MIN n k = 0)]))`;;
...
let BIGNUM_COPY_ALL = prove
 (`?f_es. !k z n x a pc es.
  ...
  ENSURES_N_SEQUENCE_TAC `pc + 0x24`
   `\s. read X0 s = word k /\
        ...
        read events s = APPEND (APPEND
          (ENUMERATEL n (\i. [EventBranch (i + 1 < n);
                              EventStore (word_add z (word (8 * i)));
                              EventLoad (word_add x (word (8 * i)))]))
          [EventBranch (n = 0)]) es` THEN
  CONJ_TAC THENL
   [ASM_CASES_TAC `n = 0` THENL
    ...
    ENSURES_N_WHILE_UP_TAC `n:num` `pc + 0x10` `pc + 0x24`
     `\i s. read X0 s = word k /\
            ...
            read events s = APPEND (APPEND
              (ENUMERATEL i (\j. [EventBranch (j + 1 < n);
                                  EventStore (word_add z (word (8 * j)));
                                  EventLoad (word_add x (word (8 * j)))]))
              [EventBranch (n = 0)]) es`
      `(\i:num. 5)` `1` `0` THEN
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [ARM_SIM'_TAC BIGNUM_COPY_EXEC [1] THEN
      ...
      REWRITE_TAC[ENUMERATEL; APPEND; CONS_11; injectivity "armevent"] THEN ASM_ARITH_TAC;
      ...
      ARM_SIM'_TAC BIGNUM_COPY_EXEC (1--5) THEN
      SUBGOAL_THEN `val (word (i + 1):int64) = i + 1` ASSUME_TAC THENL [
        ...
        ALL_TAC] THEN
      ASM_REWRITE_TAC[WORD_ITE; ENUMERATEL_ADD1; GSYM WORD_ADD; GSYM APPEND_ASSOC; APPEND] THEN
      ASM_REWRITE_TAC[ADD1; GSYM WORD_ADD; VAL_WORD_BIGDIGIT] THEN
      REWRITE_TAC[LOWDIGITS_CLAUSES] THEN ARITH_TAC;
      IMP_REWRITE_TAC[ENSURES_N_ALREADY] THEN ASM_SIMP_TAC[LOWDIGITS_SELF] THEN
  ...
  SPEC_TAC(`APPEND (ENUMERATEL n (\i. [EventBranch (i + 1 < n);
                                       EventStore (word_add z (word (8 * i)));
                                       EventLoad (word_add x (word (8 * i)))]))
                   (APPEND [EventBranch (n = 0)] es)`,`es:armevent list`) THEN
  GEN_TAC THEN

  ENSURES_N_WHILE_AUP_TAC `n:num` `k:num` `pc + 0x2c` `pc + 0x3c`
   `\i s. read X0 s = word k /\
          read X1 s = z /\
          read X4 s = word i /\
          bignum_from_memory(z,i) s = a /\
          read events s = APPEND
            (ENUMERATEL (i - n) (\j. [EventBranch ((n + j) + 1 < k); EventStore (word_add z (word (8 * (n + j))))]))
            (APPEND [EventBranch (k <= n)] es)`
   `(\i:num. 4)` `2` `0` THEN
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [REWRITE_TAC[ARITH_RULE `n - n = 0`; ENUMERATEL; APPEND] THEN
    ...
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
    REWRITE_TAC[BIGNUM_FROM_MEMORY_STEP] THEN
    ARM_SIM'_TAC BIGNUM_COPY_EXEC (1--4) THEN
    ...
    IMP_REWRITE_TAC[ENSURES_N_ALREADY] THEN SIMP_TAC[] THEN
    SUBSUMED_MAYCHANGE_TAC;
    REWRITE_TAC[NSUM_CONST_NUMSEG] THEN ASM_ARITH_TAC]);;
